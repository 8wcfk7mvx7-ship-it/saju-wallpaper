...
function CharmResultContent() {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) { router.replace("/charm"); return; }
    ...
    <main className="min-h-screen bg-[#080810] text-white">
      ...
      <div className="relative z-10 max-w-lg mx-auto px-4 pt-6 pb-32">
        ...
        <div className="flex items-center justify-between mb-6">
          <button onClick={() => router.push("/charm")} className="text-xs text-gray-600 hover:text-gray-400 transition px-3 py-1.5 rounded-full bg-white/5 border border-white/10">← 다시 분석</button>
        ...
        <button onClick={() => router.push("/charm")} className="w-full mt-4 py-3 rounded-xl border border-white/10 text-gray-600 hover:text-gray-400 text-sm transition">
          다시 분석하기
        </button>
      </div>
