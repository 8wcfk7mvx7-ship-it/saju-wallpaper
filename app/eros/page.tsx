...
const ILGAN_SEX: Record<string, { female: { power: string; energy: string; style: string }; male: { power: string; energy: string; style: string } }> = {
  ...
  임: {
    female: { power: "가장 깊고 풍부한 음기. 임수(壬水) — 큰 강물.", energy: "변화무쌍하고 깊은 음기. 어떤 상대에도 맞게 변하는 유연함.", style: "깊고 자유로운 에너지. 상대가 예측할 수 없게 만드는 타입." },
    male:   { power: "깊고 유연한 양기.", energy: "큰 강물. 변화무쌍하고 깊은 에너지.", style: "자유롭고 다양한 방식을 즐기는 타입." },
  },
  계: {
...
const SEDUCTION_TIPS: Record<string, { female: string[]; male: string[] }> = {
  ...
  임: {
    female: ["자유롭게 접근했다가 갑자기 깊어지기 — 예측 불가능하게", "절대 구속하지 말 것 — 자유를 줘야 관심을 가짐", "흥미로운 사람이 돼라 — 임수는 지루함을 못 참음"],
    male:   ["흥미롭고 다양한 면을 보여줘라", "예측 불가능하게. 항상 새로운 면을 만들어라", "자유로운 에너지로 접근하되, 결정적 순간에 깊게 들어가라"],
  },
  계: {
