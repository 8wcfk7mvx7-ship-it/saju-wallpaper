...
export default function SoloPage() {
      ...
      <main className="min-h-screen bg-[#0a0612] text-white flex flex-col page-fade-in">
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center max-w-lg mx-auto w-full px-5 py-16 text-center">
          ...
          <button onClick={() => setStep("form")}
            className="w-full py-5 rounded-2xl font-black text-lg tracking-tight bg-gradient-to-r from-indigo-700 to-violet-600 hover:from-indigo-600 hover:to-violet-500 text-white shadow-lg shadow-indigo-900/50 transition-all active:scale-[0.98]">
            내 결혼·비혼 적합도 확인하기
          </button>
          </FadeIn>
        </div>
  ...
  const GIVING_DESC: Record<string, string> = {
    ...
    경: "의리를 중요하게 여겨서, 내 사람이라고 생각되면 아낌없이 챙겨주는 편이에요. 한번 신뢰를 주면 잘 거두지 않지만, 배신을 당하면 매우 차갑게 돌아서는 타입입니다.",
    정: "조용히, 그러나 꾸준히 나누는 타입이에요. 생색내지 않고 챙겨주는 게 특징입니다. 특정한 사람에게만 깊이 마음을 쓰는 방식이라, 가까운 사람일수록 더 세심하게 챙겨요.",
    무: "넓은 마음으로 다 받아주고 베푸는 편이라, 주변에 의지하는 사람이 많아질 수 있어요. 거절을 잘 못하는 경향이 있어 에너지가 소진되지 않도록 자기 경계를 챙기는 것이 중요합니다.",
    을: "섬세하게 상대의 필요를 먼저 알아채고 채워주는 스타일이에요. 직접 표현하기보다 행동으로 보여주는 편이라, 주변에서 '말은 없는데 이상하게 챙겨준다'는 평을 듣기도 합니다.",
    신: "깐깐해 보여도 마음을 연 사람에게는 의외로 세심하게 잘 챙겨줍니다. 아무에게나 쉽게 베풀지는 않지만, 한번 마음을 주면 꼼꼼하고 일관성 있게 지속하는 편이에요.",
    기: "현실적인 선 안에서 필요한 만큼 잘 나누는, 균형 잡힌 베풂이에요. 감정보다 실용성이 앞서는 편이라, 낭비 없이 효율적으로 나누는 방식을 선호합니다.",
    임: "필요할 때 통 크게 쏘는 스타일이지만, 평소엔 계산이 빠른 쪽이에요. 분위기나 상황을 읽어 베풀기 때문에, 일관성보다 타이밍에 맞춘 베풂이 특징입니다.",
    계: "신중하게 따져보고 나누는 편이라, 베풂의 빈도 자체는 낮은 쪽입니다. 대신 한번 나눈다고 결정하면 충분하고 깊이 있게 챙겨주는, 밀도 있는 베풂을 보여줍니다.",
  };
      ...
      <div className="relative z-10 max-w-lg mx-auto px-4 pt-6 pb-16" id="solo-result">
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 mb-5">
          ...
          <p className="text-sm text-gray-300 leading-relaxed">
            {bigeopCount >= 2 || hasMuGwan
              ...
              : sikSangCount >= 2
              ? "표현력과 생산성을 의미하는 기운이 강합니다. 이 기운은 가정보다 일·창작·콘텐츠 쪽에서 발휘될 때 더 큰 결과로 이어지는 경향이 있어, 비혼 상태에서 자기 일에 몰입할 때 성취도가 높아지는 구조입니다. 이 기운이 강한 사람들은 자신이 만든 것, 쌓아온 것에서 삶의 의미를 크게 찾는 편이라, 관계보다 성취에서 더 큰 만족감을 얻는 경향이 있습니다. 비혼 생활에서 이 에너지를 창의적인 방향으로 쓸 명확한 채널을 만들어두는 것이 정서적 안정에도 중요합니다."
              : "독립성을 강하게 자극하는 기운은 두드러지지 않습니다. 비혼이 곧 더 큰 성공을 보장하는 구조는 아니며, 오히려 안정적인 관계 속에서 정서적 기반이 확보될 때 더 좋은 성과를 내는 편에 가깝습니다. 다만 그렇다고 비혼이 불리한 것도 아니므로, 본인의 라이프스타일 선호가 더 중요한 변수입니다. 이 사주에서는 결혼 여부보다, 어떤 사람과 어떤 방식으로 관계를 맺느냐가 실제 삶의 질과 성공도에 훨씬 더 큰 영향을 줍니다."}
          </p>
        </div>
        ...
        {isFullyIndependent && (
          <div className="bg-white/[0.03] border border-violet-700/20 rounded-2xl p-5 mb-5">
            ...
            <p className="text-sm text-gray-300 leading-relaxed">
              사주 안에 배우자·통제를 의미하는 기운이 거의 없고, 반면 자기 자신의 힘은 강하게 자리하고 있습니다. 이런 구조는 누군가에게 의지하거나 누군가의 보호·통제를 받을 필요 자체가 적은, 스스로 모든 걸 결정하고 만들어가는 타입입니다. 결혼이 '결핍을 채우는 선택'이 되기보다, 오히려 자유를 줄이는 선택이 될 가능성이 큽니다. 비혼으로 살아도 전혀 외롭거나 부족하지 않은, 본인 스스로가 이미 완결된 구조라고 볼 수 있어요.
            </p>
          </div>
        )}
        ...
        {isJaeDaYeoseong && (
          <div className="bg-white/[0.03] border border-amber-700/20 rounded-2xl p-5 mb-5">
            <p className="text-sm font-bold text-amber-300 mb-1">스스로 돈을 버는 구조</p>
            <p className="text-sm text-gray-300 leading-relaxed">
              여성 사주에서 재물을 의미하는 기운이 여러 개 자리한 구조입니다. 이 기운은 본래 여성에게 '내가 직접 다루는 재물·일'을 의미하는데, 이게 과하면 배우자에게 경제적으로 의존하기보다 스스로 벌고 쓰고 관리하는 쪽으로 무게가 실립니다. 경제적 독립이 이미 기본값인 구조이기 때문에, 결혼이 주는 '경제적 안정'이라는 메리트가 상대적으로 약하게 느껴질 수 있어요. 비혼으로 살아도 재정적으로 흔들릴 일이 적은, 안정적인 구조에 가깝습니다.
            </p>
          </div>
        )}
        ...
        {isChildStressRisk && (
          <div className="bg-white/[0.03] border border-rose-700/20 rounded-2xl p-5 mb-5">
            ...
            <p className="text-sm text-gray-300 leading-relaxed">
              일간이 신약한 가운데 표현력·생산성을 의미하는 기운이 매우 강한 구조입니다. 이 기운은 '내 에너지를 밖으로 쏟아내는' 기운인데, 이게 신약한 일간에서 과하게 작용하면 일상적인 소진감이 큰 편입니다. 특히 육아처럼 24시간 끊임없이 에너지를 내어줘야 하는 환경에서는 이 소진이 누적되어 정서적으로 크게 흔들릴 수 있습니다. 아이를 낳고 키우는 것이 잘 맞지 않을 수 있는 구조이므로, 비혼이나 무자녀의 삶이 정신적 안정 측면에서 훨씬 유리할 수 있습니다.
            </p>
          </div>
        )}
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 mb-5">
          ...
          <p className="text-sm text-gray-300 leading-relaxed">
            {hasMuJae
              ...
              : hasMuBigeop
              ? "자기 자신의 힘을 의미하는 기운이 약해 혼자 모든 걸 결정하고 책임져야 하는 상황에서 외로움을 크게 느낄 수 있는 구조입니다. 가족 형태의 관계가 아니더라도, 정기적으로 의지할 수 있는 친구·커뮤니티 관계를 의식적으로 만들어두는 것이 정서적 안정에 중요합니다. 비혼 생활에서 '혼자'가 '고립'이 되지 않으려면 정기적인 사람 연결이 필수입니다. 모임, 클래스, 동호회처럼 주기적으로 만나는 구조를 미리 만들어두세요."
              : "전반적으로 큰 약점이 두드러지지는 않지만, 비혼 생활은 모든 의사결정과 리스크를 혼자 감당해야 한다는 특성이 있습니다. 재물·건강·인간관계 세 영역을 정기적으로 점검하는 습관을 만들어두면 장기적으로 훨씬 안정적인 비혼 생활이 가능합니다. 특히 40대 이후의 노후 준비는 비혼일수록 더 일찍, 더 체계적으로 시작하는 것이 좋습니다. 은퇴 후의 의료·주거·사회관계망을 함께 설계해두는 것이 비혼 생활의 장기 안정성을 좌우합니다."}
          </p>
        </div>
        ...
        {(() => {
          ...
          const GROUP_LABEL: Record<string, string> = { 관성: "배우자·책임 기운", 식상: "표현·생산 기운", 인성: "보호·안정 기운" };
          return (
            <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 mb-5">
              <p className="text-sm font-bold text-indigo-300 mb-3">기운 세력 — 관계 준비도 진단</p>
              ...
              <p className="text-xs text-gray-500 leading-relaxed pt-2 border-t border-white/5">
                {gwan?.status === "무" || gwan?.status === "약함"
                  ? `배우자·사회적 책임을 의미하는 기운이 ${gwan.status}한 구조예요. 누군가에게 통제받거나 의존하는 관계보다 스스로 결정하고 움직이는 삶이 더 에너지가 나는 타입이에요. 비혼이 심리적으로 훨씬 자연스럽게 느껴질 수 있어요.`
                  : gwan?.status === "강함"
                  ? `배우자·책임 기운이 강해 관계·책임·사회적 연결에 대한 욕구가 크게 자리하고 있어요. 완전한 비혼보다 가까운 파트너십 관계를 유지하면서 자유도를 확보하는 방식이 현실적으로 잘 맞을 수 있어요.`
                  : `배우자·책임 기운이 보통 수준이라, 결혼·비혼 어느 쪽이든 본인의 라이프스타일 선호와 상대방의 궁합이 훨씬 더 큰 변수예요.`}
              </p>
            </div>
          );
        })()}
        ...
        <ResultFooterActions targetId="solo-result" fileName="결혼_비혼_적합도" />
      </div>
    </main>
  );
}
