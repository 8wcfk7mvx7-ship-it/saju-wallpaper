...
const TYPES: { id: string; title: string; emoji: string; cond: (c: Record<string, number>, strength: string) => boolean; desc: string; warn: string; jobs: string[]; action: string; envTitle: string; env: string }[] = [
  {
    ...
    warn: "다만 조직·규율의 기운이 없다는 건 외부의 견제와 규칙이 약하다는 뜻이기도 합니다. 세금·계약·법적 절차처럼 '하기 싫지만 반드시 해야 하는 행정'을 미루지 않는 시스템(세무사, 자동화 툴)을 처음부터 만들어두는 것이 사업의 수명을 좌우합니다. 또 하나의 함정은 '아이디어 과잉'입니다. 이 기운이 강하면 새 아이디어가 계속 떠올라 기존 사업을 중간에 접고 새것을 시작하는 사이클을 반복하기 쉽습니다. 한 가지를 최소 6개월 이상 지속한 뒤 평가하는 기준을 만들어두세요.",
  ...
  },
  {
    ...
    warn: "자유로운 환경일수록 스스로 만든 루틴이 없으면 쉽게 무너집니다. '어디서든 일한다'와 '아무 때나 일한다'는 다릅니다. 시간대를 고정한 최소한의 루틴을 만들어야 자유가 오히려 생산성으로 이어집니다. 또한 인성(학습력)이 강한 유형일수록 '공부는 계속 하는데 정작 수익으로 연결하는 단계를 계속 미루는' 패턴이 반복되기 쉽습니다. 배움과 실행 사이의 비율이 5:5 이하가 되도록 의식적으로 조정해야 합니다.",
  ...
  },
  {
    ...
    warn: "돈을 굴리는 기운이 강한 만큼 욕심도 함께 커질 수 있습니다. 특히 기운이 단단한 상태에서 거기에만 몰두하면 한 곳에 자산을 몰아넣는 '몰빵' 성향이 나오기 쉬우니, 투자 비중을 미리 정해두고 그 기준을 지키는 것이 핵심입니다. 재성 기운이 매우 강한 경우 '빠른 수익'에 집착해 검증되지 않은 고수익 상품에 끌리는 경향도 생깁니다. '이번 기회는 진짜다'라는 감이 들수록 한 박자 쉬어가야 합니다. 큰 수익보다 손실 회피가 장기 자산 형성의 핵심입니다.",
  ...
  },
  {
    ...
    warn: "그렇다고 부업·투자가 안 맞는다는 뜻은 아닙니다. 다만 본업의 안정성을 깨뜨릴 정도의 모험은 피하는 것이 좋고, 사업·투자는 '본업을 지킨 채로' 천천히 비중을 늘려가는 방식이 이 사주에는 훨씬 잘 맞습니다. 이 유형에서 가장 흔히 실패하는 패턴은 주변의 '창업·투자 성공담'에 자극받아 갑자기 큰 결단을 내리는 것입니다. 한 번에 바꾸려 하기보다, 1~2년에 걸쳐 단계적으로 전환을 준비하면 훨씬 높은 성공률을 보입니다.",
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center max-w-lg mx-auto w-full px-5 py-16 text-center">
          ...
          <FadeIn delay={300} className="w-full">
            ...
            <button onClick={() => setStep("form")}
              className="w-full py-5 rounded-2xl font-black text-lg tracking-tight bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white shadow-lg shadow-cyan-900/50 transition-all active:scale-[0.98]">
              내 일 유형 확인하기
            </button>
          </FadeIn>
        </div>
      ...
      <div className="relative z-10 max-w-lg mx-auto px-4 pt-6 pb-16" id="nomad-result">
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 mb-8">
          ...
          <div className="text-sm text-gray-300 leading-relaxed space-y-2">
            <p>
              새로운 것을 만드는 힘, 그것을 돈으로 바꾸는 힘, 체계와 규칙을 지키는 힘, 학습과 전문성을 쌓는 힘, 추진력과 경쟁심을 의미하는 힘. 이 다섯 기운의 균형이 곧 일하는 방식의 취향을 결정하는데, 단순히 글자 개수만이 아니라 그 글자가 어느 자리에 있고 주변 기운과 어떤 관계를 맺는지까지 함께 봐야 진짜 세력이 보여요. 가장 강한 두 기운이 시너지를 이룰 때 커리어가 가장 빠르게 성장하고, 가장 약한 기운이 필요한 업무를 강요받을 때 번아웃이 찾아옵니다. 어떤 회사·역할·환경이 내 두 강한 기운에 불을 붙여주는지를 기준으로 커리어 결정을 내리면, 억지로 버티는 일 없이 자연스럽게 성과를 낼 수 있습니다.
            </p>
        ...
        <ResultFooterActions targetId="nomad-result" fileName="디지털노마드" />
      </div>
    </main>
  );
}
