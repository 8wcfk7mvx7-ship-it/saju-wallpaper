...
const EL_DATA: Record<string, ElData> = {
  목: {
    ...
    coinAffinity: 68,
    lossPattern: "\"이번엔 다르다\"를 반복하며 손절 타이밍을 계속 미루다 대규모 손실. 성장성에 대한 확신이 강한 만큼, 실적 발표나 악재 뉴스에도 '곧 올라오겠지'라는 낙관 편향이 지속되는 것이 핵심 함정입니다. 손절 기준선(-8~10%)을 매수 직후 반드시 설정하고 그 선을 넘으면 이유 불문하고 실행하는 원칙이 필요합니다.",
    buyDipTendency: 55,
  ...
  화: {
    ...
    coinAffinity: 85,
    lossPattern: "고점에서 뇌동매수 → 폭락 → 패닉셀. 반복적 고점 물량 패턴. 분위기에 민감한 기질상 '지금 안 사면 기회를 놓친다'는 FOMO 심리가 가장 강하게 작동합니다. 매수 전 '3일 후에도 같은 결정을 내릴 것인가?'를 자문하는 습관이 고점 뇌동매수를 줄이는 가장 실용적인 방법입니다.",
    buyDipTendency: 30,
  ...
  토: {
    ...
    coinAffinity: 25,
    lossPattern: "\"손해 아니니까 안 팔아도 돼\" 마인드로 수익 기회를 놓치거나 오랫동안 손실 방치. 안정을 추구하는 기질상 '팔았다가 더 오르면 어쩌나'라는 두려움도 함께 작용해 매도 결정 자체를 계속 미루게 됩니다. 매수할 때 목표 수익률(+20%)과 손절 라인(-10%)을 미리 적어두고, 어느 한 기준에 닿으면 자동으로 매도하는 조건부 주문을 활용하면 이 함정을 피할 수 있습니다.",
    buyDipTendency: 82,
  ...
  금: {
    ...
    coinAffinity: 38,
    lossPattern: "분석에 너무 오래 걸려 매수 타이밍 상실, 또는 분석 후 과신으로 집중 투자 손실. 본인의 분석 결과에 대한 확신이 강한 만큼, 시장이 반대로 움직여도 '내 분석이 맞을 것'이라며 버티다 손실이 커지는 구조가 대표적인 패턴입니다. 분석은 도구이지 답이 아닙니다 — 어떤 분석도 틀릴 수 있다는 전제 아래 포지션 크기를 조절하는 것이 금 기운 투자자에게 가장 중요한 규칙입니다.",
    buyDipTendency: 74,
  ...
  수: {
    ...
    coinAffinity: 60,
    lossPattern: "정보 과부하로 결정 지연 → 과도한 종목 교체 → 수익 없이 거래 비용만 증가. 정보를 넓게 수집하는 능력이 강한 반면, 수많은 가능성 사이에서 결정을 내리지 못하고 포트폴리오를 계속 조정하다 결국 수익을 내지 못하는 것이 핵심 함정입니다. '더 나은 정보가 나오면 판단하겠다'는 생각이 가장 위험합니다 — 매월 1회 이상의 매매는 하지 않는 규칙을 미리 정해두세요.",
    buyDipTendency: 65,
...
export default function StockPage() {
  ...
  const [birthTime, setBirthTime] = useState<BirthTimeValue>({
    hour: 11, minute: 4, unknown: false, useJajasi: false,
  });
            ...
            <div>
                ...
                <div className="flex overflow-hidden rounded-lg border border-white/10">
                  {(["solar","lunar"] as const).map(t => (
                    <button key={t} type="button" onClick={() => { setCalendarType(t); setIsLeapMonth(false); setForm(f => ({ ...f, birthMonth: "", birthDay: "" })); }}
                      className={`px-4 py-1.5 text-sm font-medium transition ${calendarType === t ? "bg-emerald-600 text-white" : "text-gray-400 hover:bg-white/5"}`}>
                      {t === "solar" ? "양력" : "음력"}
                    </button>
                  ))}
                </div>
              ...
              {calendarType === "lunar" && (
                <label className="flex items-center gap-2 mt-3 cursor-pointer">
                  <input type="checkbox" checked={isLeapMonth} onChange={e => setIsLeapMonth(e.target.checked)} className="w-4 h-4 rounded accent-emerald-500" />
