...
const KR_CITY_BY_ELEMENT: Record<string, {
  ...
  목: {
    cities: [
      { name: "강원 양양", reason: "울창한 숲과 서핑 해변이 공존하는 도시. 목(木)의 상승 에너지가 충만해 창의력과 성장 기운이 살아납니다.", emoji: "🌲", neighborhoods: "양양 서퍼비치, 하조대", food: "오징어, 감자전, 송이버섯" },
      { name: "전북 전주", reason: "전통문화와 역사가 살아있는 도시. 목의 인문학적 기운이 강합니다. 글쓰기·예술에 날개를 달아줍니다.", emoji: "📜", neighborhoods: "한옥마을, 객리단길", food: "비빔밥, 콩나물국밥, 막걸리" },
    ],
    avoid: "목이 과다하면 금(金) 기운의 도시(인천·울산)는 충돌을 일으킬 수 있습니다.",
  },
  화: {
    cities: [
      ...
      { name: "전남 여수", reason: "밤바다 야경과 강렬한 햇살이 공존하는 항구도시. 화려한 빛과 열기의 화(火) 기운이 강해 사람을 끌어모으는 인기·매력 운을 키워줍니다.", emoji: "🌇", neighborhoods: "여수 밤바다, 돌산도, 이순신광장", food: "갓김치, 게장, 돌산갓" },
    ],
  ...
  토: {
    cities: [
      ...
      { name: "경북 대구·경주·포항·안동", reason: "메마르고 단단하게 다져진 대지의 토 기운. 비옥한 황토보다는 건조하고 굳센 흙의 기운이라 끈기와 뚝심, 전통을 지키는 힘을 키워줍니다.", emoji: "🪨", neighborhoods: "동성로, 불국사·대릉원, 포스코 일대, 하회마을", food: "막창, 황남빵, 물회, 헛제삿밥" },
    ],
    avoid: "토가 과다하면 목(木) 기운의 도시(강원 양양·전북 전주)에서 충돌이 생길 수 있습니다.",
  },
  금: {
    cities: [
  ...
  수: {
    cities: [
      { name: "강원 강릉·속초·동해", reason: "동해 바다와 청정 계곡이 펼쳐진 해안 도시들. 수(水)의 지혜와 흐름 에너지가 직관력과 창의력을 높입니다.", emoji: "🌊", neighborhoods: "안목해변, 정동진, 경포대, 속초 중앙시장", food: "초당 순두부, 막국수, 황태, 물회" },
...
const WORLD_BY_ELEMENT: Record<string, {
  ...
  수: {
    countries: [
      ...
      { name: "핀란드", flag: "🇫🇮", reason: "임수(壬水)의 나라. 수많은 호수와 깊은 겨울 어둠이 만든 차분하고 묵직한 수 기운. 깊이 있는 사고와 복지·디자인·IT 연구 분야에서 강한 운이 따릅니다.", cities: "헬싱키, 탐페레, 오울루", vibe: "호수·고요·연구" },
      { name: "일본 홋카이도", flag: "🇯🇵", reason: "임수(壬水)의 차가운 설원 지대. 일본 본토와는 다른 광활하고 차분한 수 기운이 강해 휴식·치유·연구 분야에 유리합니다.", cities: "삿포로, 오타루, 아사히카와", vibe: "설원·치유·고요" },
      { name: "캐나다 북부", flag: "🇨🇦", reason: "임수(壬水)의 광대한 동토와 호수 지대. 캐나다 남부의 자유로운 목 기운과 달리, 깊고 차가운 수의 저장·지혜 에너지가 강합니다.", cities: "유콘, 노스웨스트준주, 누나부트", vibe: "동토·지혜·고독" },
    ],
  },
};
...
const CITY_ELEMENT_MAP: Record<string, string> = {
  ...
  인천: "금", 부산: "화", 대구: "토", 경주: "토", 광주: "화", 대전: "토", 울산: "금", 세종: "토",
  강릉: "수", 동해: "수", 속초: "수", 양양: "목", 춘천: "수", 가평: "수",
  전주: "목", 안동: "토", 공주: "토", 부여: "토", 이천: "토", 여주: "토",
  순천: "토", 담양: "토", 여수: "화", 제주: "화", 통영: "화", 거제: "금", 포항: "토",
};
