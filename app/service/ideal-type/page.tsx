...
const IDEAL: Record<string, { type: string; desc: string; trait: string; warn: string }> = {
  갑: { type: "조용히 리드해주는 안정적인 사람", desc: "갑목인 당신은 자존심이 강하고 곧은 성격이라, 본인을 다그치지 않으면서도 묵묵히 중심을 잡아주는 상대에게 끌립니다. 화려한 말보다 행동으로 신뢰를 주는 사람이 진짜 이상형이에요.", trait: "잔소리 없이 믿어주는 사람", warn: "너무 강하게 통제하려는 사람은 오히려 멀어지게 만듭니다." },
  을: { type: "내 편이 되어주는 다정한 사람", desc: "을목인 당신은 유연하지만 속은 여립니다. 내 감정을 먼저 알아채고 챙겨주는, 다정하고 섬세한 사람에게 강하게 끌려요. 무뚝뚝한 사람보다 표현 잘하는 사람이 잘 맞습니다.", trait: "표현을 아끼지 않는 따뜻한 사람", warn: "감정 기복이 심한 사람과는 쉽게 지칠 수 있어요." },
  병: { type: "내 에너지를 받아주는 시원한 사람", desc: "병화인 당신은 밝고 직진하는 스타일이라, 그 에너지를 부담스러워하지 않고 함께 즐겨줄 사람이 이상형입니다. 너무 차분하고 반응 없는 상대는 답답하게 느껴질 수 있어요.", trait: "리액션이 좋고 같이 노는 사람", warn: "지나치게 진지하고 무거운 사람은 숨 막힐 수 있어요." },
  정: { type: "내 마음을 깊이 들여다봐 주는 사람", desc: "정화인 당신은 은은하지만 감정이 깊습니다. 겉모습보다 내면을 봐주고, 천천히 다가와 주는 사람에게 마음을 엽니다. 빠르게 들이대는 사람은 오히려 부담스러워요.", trait: "천천히, 진심으로 다가오는 사람", warn: "조급하게 관계를 밀어붙이는 사람은 피하게 됩니다." },
  무: { type: "큰 변화 없이 곁을 지켜주는 사람", desc: "무토인 당신은 안정과 신뢰를 가장 중요하게 여깁니다. 변덕 없이 한결같이 곁에 있어주는 사람, 약속을 지키는 사람이 진짜 이상형이에요.", trait: "한결같고 약속을 지키는 사람", warn: "즉흥적이고 변덕스러운 사람과는 오래가기 어려워요." },
  기: { type: "디테일까지 챙겨주는 섬세한 사람", desc: "기토인 당신은 꼼꼼하고 완벽주의 성향이 있어, 작은 부분까지 신경 써주는 섬세한 사람에게 끌립니다. 대충대충 하는 사람과는 잘 안 맞아요.", trait: "사소한 것도 기억해주는 사람", warn: "무신경하고 둔감한 사람은 답답하게 느껴집니다." },
  경: { type: "솔직하고 거짓 없는 사람", desc: "경금인 당신은 직선적이고 솔직한 성격이라, 똑같이 솔직하게 말해주는 사람을 가장 편하게 느낍니다. 돌려 말하거나 눈치 보는 사람은 답답해요.", trait: "할 말은 하는 시원한 사람", warn: "이중적이거나 뒤에서 다른 말 하는 사람을 가장 싫어해요." },
  신: { type: "설렘을 계속 만들어주는 사람", desc: "신금인 당신은 섬세하고 낭만을 추구합니다. 매너 있고 센스 있게 분위기를 만들어주는 사람, 지루하지 않은 사람에게 끌립니다.", trait: "센스 있고 분위기를 잘 만드는 사람", warn: "너무 무던하고 평범한 루틴만 반복하는 사람은 매력을 못 느껴요." },
  임: { type: "내 자유를 인정해주는 사람", desc: "임수인 당신은 구속받는 걸 싫어합니다. 간섭하지 않으면서도 묵직하게 신뢰를 주는 사람, 각자의 영역을 존중해주는 사람이 이상형이에요.", trait: "집착하지 않고 믿어주는 사람", warn: "사사건건 확인하고 통제하려는 사람은 숨 막혀 합니다." },
  계: { type: "내 마음을 이해해주는 공감형 사람", desc: "계수인 당신은 감수성이 풍부합니다. 말하지 않아도 내 기분을 알아채고, 깊이 공감해주는 사람에게 강하게 끌립니다.", trait: "공감 능력이 뛰어난 사람", warn: "공감 없이 해결책만 제시하는 사람은 서운하게 느껴져요." },
};
...
export default function IdealTypePage() {
      ...
      <main className="min-h-screen bg-[#0a0612] text-white flex flex-col">
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center max-w-lg mx-auto w-full px-5 py-16 text-center">
          ...
          <h1 className="text-4xl font-black mb-4 leading-tight tracking-tight">
            내가 진짜<br />
            <span className="text-fuchsia-400">끌리는 사람</span>은?
          </h1>
          <p className="text-gray-400 text-base mb-2 leading-relaxed">
            내가 의식적으로 좋다고 말하는 타입 말고,<br />
            <span className="text-gray-300 font-medium">사주에 새겨진 진짜 이상형</span>을 확인하세요.
          </p>
          <p className="text-gray-600 text-sm mb-12">
            생년월일만 입력하면 1분 안에 결과가 나옵니다
          </p>

          <div className="w-full space-y-3 mb-10 text-left">
            {[
              ["일간 기반 무의식적 끌림", "이성적으로 고른 타입과 실제로 끌리는 타입은 다릅니다"],
              ["내가 이끌리는 성향 키워드", "왜 항상 비슷한 사람만 만나는지 이유가 보입니다"],
              ["피해야 할 상대 유형", "반복되는 연애 패턴의 원인을 짚어드립니다"],
            ].map(([title, desc]) => (
      ...
      <main className="min-h-screen bg-[#0a0612] text-white">
        ...
        <div className="relative z-10 max-w-lg mx-auto px-4 pt-6 pb-24">
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 mb-5">
            <BirthInputForm value={form} onChange={setForm} label="나의 정보" accent="#d946ef" />
          </div>
  ...
  if (step === "loading") {
    return <AnalysisLoading subject="나의 진짜 이상형" duration={2200} onDone={() => setStep("result")} />;
  }
  ...
  return (
    <main className="min-h-screen bg-[#0a0612] text-white">
      ...
      <div className="relative z-10 max-w-lg mx-auto px-4 pt-6 pb-16">
        <div className="text-center mb-8">
          <p className="text-fuchsia-400 text-xs font-bold tracking-widest mb-2">YOUR TRUE IDEAL TYPE</p>
          <h1 className="text-2xl font-black leading-snug">
            {ilgan}{r.pillarsDetail.day.jj}일주, 당신이 진짜 끌리는 사람은
          </h1>
        </div>
