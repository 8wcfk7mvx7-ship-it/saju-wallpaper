...
const UUNSEONG_FORTUNE: Record<string, { label: string; score: number; color: string; desc: string; narrative: string; caution: string; keyword: string }> = {
  ...
  건록: {
    ...
    narrative:"건록은 공무원이 정식으로 녹봉을 받는 상태다. 실력·지위·재물이 가장 균형 잡혀 있고, 해온 일들이 결실을 맺는 구간이다. 직장인은 핵심 인력으로 인정받고, 사업가는 탄탄한 수익 구조가 만들어지며, 관계는 믿을 수 있는 사람들로 정리돼. 억지로 욕심내지 않아도 자연스럽게 채워지는 시기라 심리적으로도 가장 안정적이다.",
    caution:"너무 편안한 나머지 도전을 멈추면 다음 쇠·병 대운의 하락이 더 크게 느껴질 수 있어. 이 시기에 체력과 자산을 비축해두는 것이 중요하다.",
