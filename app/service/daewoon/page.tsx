...
const UUNSEONG_FORTUNE: Record<string, { label: string; score: number; color: string; desc: string; narrative: string; caution: string; keyword: string; finance: string; career: string; love: string; health: string }> = {
  ...
  제왕: {
    ...
    desc:"에너지가 최고조에 달한 왕의 시기. 그러나 정점 이후엔 내리막이 시작돼요.",
  ...
  쇠: {
    ...
    desc:"에너지가 꺾이기 시작하는 시기. 무리보다 내실을 다져야 해요.",
  ...
  병: {
    ...
    desc:"몸과 마음이 본격적으로 지치는 시기. 과욕을 버리고 쉬어야 해요.",
  ...
  묘: {
    ...
    desc:"무덤에 갇힌 형국. 활동 반경이 좁고 정신적 답답함이 극에 달해요.",
  ...
  절: {
    ...
    desc:"모든 기반이 끊어지고 리셋되는 시기. 고독과 단절이 운명처럼 찾아와요.",
...
const SIPSEONG_DAEWOON: Record<string, { title: string; color: string; desc: string; life: string; caution: string }> = {
  ...
  상관: {
    ...
    desc:"직장·규범·권위를 흔드는 기운. 직장·규범·권위에 반발하는 시기. 창의적이지만 파란이 많다.",
  ...
  정재: {
    ...
    desc:"성실하게 쌓이는 재물의 대운. 꾸준한 노력이 착실한 자산으로 변환돼요.",
  ...
  정인: {
    ...
    desc:"배움과 인정의 대운. 안정적인 도움과 지지를 받는 시기.",
