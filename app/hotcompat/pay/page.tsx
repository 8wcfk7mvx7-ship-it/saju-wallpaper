...
function HotCompatPayContent() {
  ...
  async function handleCardPayment() {
    ...
    try {
      ...
      await payment.requestPayment({
        ...
        successUrl: `${base}/hotcompat/success?orderId=${orderId}`,
        failUrl: `${base}/hotcompat/pay?orderId=${orderId}&error=true`,
        customerName: "고객",
      });
    } catch (e: unknown) {
  ...
  function handleBlueberryPayment() {
    ...
    if (blueberries < BLUEBERRY_PRICE) {
      setError(`별조각이 부족합니다. 현재 ${blueberries.toLocaleString()}개 / 필요 ${BLUEBERRY_PRICE.toLocaleString()}개`);
      return;
    }
    ...
    router.replace("/hotcompat/success?method=blueberry");
  }
