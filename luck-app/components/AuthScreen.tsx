...
export default function AuthScreen({ onBack, onAuthed }: { onBack: () => void; onAuthed: () => void }) {
  ...
  async function handleOAuth(fn: () => Promise<{ error: string | null }>) {
    ...
    // 성공 시 OAuth 제공자 화면으로 리다이렉트되므로, 이 화면에서 별도로 onAuthed를 부를 필요는 없음
  ...
  return (
    <main className="min-h-screen page-fade-in" style={{ background: "var(--bg)" }}>
      <div className="max-w-lg mx-auto px-6 pt-10 pb-16">
        <button onClick={onBack} className="text-sm font-bold mb-4" style={{ color: "var(--ink-soft)" }}>
          ← 뒤로
        </button>

        <div className="text-center mb-8">
          <CloverIcon size={40} className="mx-auto mb-3 float-leaf" />
        ...
        {!configured && (
          <div className="retro-card p-4 mb-5" style={{ background: "rgba(178,58,46,0.08)" }}>
            <p className="text-xs leading-relaxed" style={{ color: "var(--ink-soft)" }}>
              아직 로그인 기능이 서버에 연결되지 않았어요. 개발자가 클라우드 설정을 마치면 이용할 수 있어요.
            </p>
          </div>
        )}
        ...
        <div className="space-y-2.5 mb-5">
          <button
            ...
            className="retro-btn w-full py-3.5 text-sm font-bold flex items-center justify-center gap-2"
            style={{ background: "#000", color: "#fff", opacity: configured ? 1 : 0.5 }}>
            <AppleGlyph /> Apple로 계속하기
          </button>
          <button
            disabled={!configured}
            onClick={() => handleOAuth(signInWithGoogle)}
            className="retro-btn w-full py-3.5 text-sm font-bold flex items-center justify-center gap-2"
            style={{ background: "#fff", color: "#3c4043", border: "2px solid #dadce0", opacity: configured ? 1 : 0.5 }}>
            <GoogleGlyph /> Google로 계속하기
          </button>
        </div>

        <div className="flex items-center gap-3 mb-5">
          <div className="flex-1 h-px" style={{ background: "var(--card-border)", opacity: 0.3 }} />
          <span className="text-xs" style={{ color: "var(--ink-soft)" }}>또는</span>
          <div className="flex-1 h-px" style={{ background: "var(--card-border)", opacity: 0.3 }} />
        </div>
        ...
        {error && <p className="text-xs mt-3 leading-relaxed" style={{ color: "#b23a2e" }}>{error}</p>}
        {info && <p className="text-xs mt-3 leading-relaxed" style={{ color: "var(--clover)" }}>{info}</p>}

        <button
          onClick={handleEmailSubmit}
          disabled={!configured || loading}
          className="retro-btn font-display w-full py-3.5 text-base mt-4"
        ...
        <button
          ...
          className="w-full text-center text-xs py-3 mt-2 underline underline-offset-4" style={{ color: "var(--ink-soft)" }}>
